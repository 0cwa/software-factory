...
export class PlanChangeRuntime {
  ...
  private async dispatch(store: RuntimeRepository, run: FactoryRun, phaseId: "scout" | "architect", input: unknown, before: RepositoryIdentity): Promise<{ status: "succeeded"; output: unknown; envelope: PhaseEnvelope; run: FactoryRun } | { status: "failed" | "outcome_unknown"; run: FactoryRun }> {
    ...
    const attemptId = `${run.id}-${phaseId}-${randomUUID()}`; const timeout = Math.min(Math.max(this.options.dispatchTimeoutMs ?? 5_000, 1), RUNTIME_LIMITS.maxDispatchTimeoutMs); const deadlineEpochMs = Date.now() + timeout; const deadlineMonotonicMs = performance.now() + timeout; const invocation = { runId: run.id, phaseId, target, input, inputDigest: jsonDigest(input), workflowDigest: this.workflowDigest, prompt: promptRef(this.options.catalog, phaseId), repositoryIdentityDigest: jsonDigest(before), environmentIdentityDigest: this.environment.digest, deadline: deadlineEpochMs };
    ...
    const grace = Math.min(Math.max(this.options.dispatchGraceMs ?? 100, 0), RUNTIME_LIMITS.maxDispatchGraceMs); const controller = new AbortController();
    const portPromise: Promise<unknown> = Promise.resolve().then(() => this.options.capabilityPort.dispatch(target, { ...invocation, signal: controller.signal })); let timer: NodeJS.Timeout | undefined; let graceTimer: NodeJS.Timeout | undefined; let timedOut = false; let settled = false;
    const resultPromise = new Promise<unknown>((resolveResult) => {
      ...
      timer = setTimeout(() => { timedOut = true; controller.abort(); graceTimer = setTimeout(() => settle({ status: "outcome_unknown" }), grace); }, timeout);
      portPromise.then((result) => settle(result), () => settle({ status: "outcome_unknown" }));
    });
    let raw: unknown; try { raw = await resultPromise; } finally { if (timer) clearTimeout(timer); if (graceTimer) clearTimeout(graceTimer); }
