...
export interface PlanChangeRequest { readonly requestId: string; readonly task: string; readonly constraints?: readonly string[]; }
...
function validateReceipt(raw: unknown, target: string, expectedState?: string): CapabilityReceiptRef | undefined {
  ...
  if (!states.has(raw.state as string) || (expectedState && raw.state !== expectedState)) return undefined;
...
function receiptFor(result: CapabilityDispatchResult, target: string): CapabilityReceiptRef | undefined {
  if (result.status === "outcome_unknown") return undefined;
  try { return validateReceipt(result.receipt, target, result.status === "succeeded" ? "succeeded" : "failed"); } catch { return undefined; }
}
...
function reconcileEvents(events: readonly JournalEvent[], run: FactoryRun | undefined, diagnostics: Diagnostic[]): FactoryRun {
  ...
  for (const event of events.slice(1)) {
    ...
    if (event.type === "phase.result") {
      ...
      if (activeAttempt) { const result = capabilityResults.get(activeAttempt); if (!result || result.data.status !== event.data.status) divergent("Phase result does not reconcile with its capability result"); if (event.data.status === "succeeded") successfulAttempts.set(activeAttempt, event); activeAttempt = undefined; activePhase = undefined; }
...
export class PlanChangeRuntime {
  ...
  private async dispatch(store: RuntimeRepository, run: FactoryRun, phaseId: "scout" | "architect", input: unknown, before: RepositoryIdentity): Promise<{ status: "succeeded"; output: unknown; envelope: PhaseEnvelope; run: FactoryRun } | { status: "failed" | "outcome_unknown"; run: FactoryRun }> {
    ...
    const attemptId = `${run.id}-${phaseId}-${randomUUID()}`; const invocation = { runId: run.id, phaseId, target, input, inputDigest: jsonDigest(input), workflowDigest: this.workflowDigest, prompt: promptRef(this.options.catalog, phaseId), repositoryIdentityDigest: jsonDigest(before), environmentIdentityDigest: this.environment.digest };
    ...
    const timeout = Math.min(Math.max(this.options.dispatchTimeoutMs ?? 5_000, 1), RUNTIME_LIMITS.maxDispatchTimeoutMs); const grace = Math.min(Math.max(this.options.dispatchGraceMs ?? 100, 0), RUNTIME_LIMITS.maxDispatchGraceMs); const controller = new AbortController();
    const portPromise: Promise<unknown> = Promise.resolve().then(() => this.options.capabilityPort.dispatch(target, { ...invocation, signal: controller.signal })); let timer: NodeJS.Timeout | undefined; let timedOut = false;
    const resultPromise = new Promise<unknown>((resolveResult) => { timer = setTimeout(() => { timedOut = true; controller.abort(); setTimeout(() => resolveResult({ status: "outcome_unknown" }), grace); }, timeout); portPromise.then((result) => { if (!timedOut) resolveResult(result); }, () => { if (!timedOut) resolveResult({ status: "outcome_unknown" }); }); });
    let raw: unknown; try { raw = await resultPromise; } finally { if (timer) clearTimeout(timer); }
    let normalized: { status: "succeeded" | "failed" | "outcome_unknown"; output?: unknown; receipt?: CapabilityReceiptRef; code: string };
    try {
      if (!isRecord(raw) || !["succeeded", "failed", "outcome_unknown"].includes(raw.status as string)) normalized = { status: "outcome_unknown", code: "capability.malformed-result" };
        ...
        if (raw.diagnostics !== undefined && (!Array.isArray(raw.diagnostics) || raw.diagnostics.length > 64 || raw.diagnostics.some((item) => !isRecord(item) || typeof item.code !== "string" || item.code.length > 256))) normalized = { status: "outcome_unknown", code: "capability.malformed-result" };
        else normalized = { status: "outcome_unknown", code: "capability.outcome-unknown" };
      } else if (raw.status === "failed") {
        if (!Array.isArray(raw.diagnostics) || raw.diagnostics.length > 64 || raw.diagnostics.some((item) => !isRecord(item) || typeof item.code !== "string" || item.code.trim().length === 0 || item.code.length > 256)) normalized = { status: "outcome_unknown", code: "capability.malformed-result" };
        else if (raw.receipt !== undefined && !validateReceipt(raw.receipt, target, "failed")) normalized = { status: "outcome_unknown", code: "capability.malformed-result" };
        else normalized = { status: "failed", code: (raw.diagnostics as Record<string, unknown>[]).map((item) => item.code as string).slice(0, 16).join("|") || "capability.failed" };
      } else {
        ...
        else if (outputBytes > phase.maxOutputBytes) normalized = { status: "failed", code: "capability.output-bound" };
    ...
    const settledStatus = timedOut ? "outcome_unknown" : normalized.status; const code = timedOut ? "capability.outcome-unknown" : normalized.code; const receipt = settledStatus === "succeeded" ? normalized.receipt : undefined;
    ...
    if (settledStatus !== "succeeded") { const failedRun: FactoryRun = { ...run, status: "failed", executionStatus: settledStatus, uncertainInvocation: settledStatus === "outcome_unknown" }; await store.append("phase.result", { phaseId, attemptId, status: settledStatus, outputDigest: "", diagnosticCodes: code ? [code] : [] }); await store.writeRun(failedRun); return { status: settledStatus, run: failedRun }; }
