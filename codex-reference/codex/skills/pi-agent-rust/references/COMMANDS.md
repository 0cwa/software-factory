...
```bash
PI_AGENT_WORK_DIR="${PI_AGENT_WORK_DIR:-${XDG_CACHE_HOME:-${HOME}/.cache}/pi_agent_rust}"
PI_AGENT_RUNTIME_DIR="${XDG_RUNTIME_DIR:-${PI_AGENT_WORK_DIR}}"
export CARGO_TARGET_DIR="${CARGO_TARGET_DIR:-${PI_AGENT_WORK_DIR}/target}"
export TMPDIR="${TMPDIR:-${PI_AGENT_RUNTIME_DIR}/tmp}"
...
bash install.sh --yes --offline --version v0.0.0 \
  --artifact-url "<artifact-file-url>" \
  --checksum "<sha256>" \
